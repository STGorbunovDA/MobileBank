...
namespace MobileBank.Forms
{
    public partial class InternetTVPayments : Form
    {
        ...
        void InternetTV_Load(object sender, EventArgs e)
        {
            ...
            SettingMethod.Loading_serviceTypeInternetTV_cmd(cmb_servicesInternetTVPayments);
        }
        ...
        void Btn_TransferPaymentsInternetTV_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(txB_sum.Text))
                {
                    if (txB_cardCvv.Text != "")
                    {
                        if (txB_personallPaymentsInternetTV.Text != "")
                        {
                            ...
                            if (DataStorage.attemptsPin < 3)
                            {
                                if (InternetСheck.CheackSkyNET())
                                {
                                    ...
                                    var queryTransaction2 = $"INSERT INTO transactions (transaction_type, transaction_destination, transaction_date, " +
                                        $"transaction_number, transaction_money, transaction_currency, id_bank_card) VALUES ('Оплата {cmb_servicesInternetTVPayments.Text}', " +
                                                        ...
                                                        if (commandTransfer3.ExecuteNonQuery() == 1)
                                                        {
                                                            using (MySqlCommand commandTransfer4 = new MySqlCommand(queryTransaction4, DataBaseConnection.GetInstance.GetConnection()))
                                                            {
                                                                if (commandTransfer4.ExecuteNonQuery() == 1)
                                                                {
                                                                    ...
                                                                    MessageBox.Show($"Оплата {cmb_servicesInternetTVPayments.Text} произведена", "Успешно", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                                }
                                                                else
                                                                {
